...
- **Заявки уходят на модерацию.** `POST /listings` создаёт объявление со статусом
  ...
  Одобрение пока делается руками: `UPDATE listings SET status='published' WHERE id=...`.
...
Личного кабинета, редактирования и удаления своих объявлений, интерфейса модерации,
