...
/**
 * Текст, по которому идёт поиск. Worker кладёт его в отдельную колонку при записи,
 * чтобы SQL-поиск давал ровно тот же результат, что и фильтрация в браузере.
 */
...
/** Слова запроса: совпадение считается, когда найдены все. */
...
export function matchesFilters(listing, filters) {
  ...
  if (f.q) {
    ...
    if (!searchWords(f.q).every((word) => text.includes(word))) return false;
  }
  return true;
}
...
/* ------------------------- проверка заявки от человека ---------------------
 * Парсеру доверия больше — его данные только нормализуются. Заявку от человека
 * проверяем по одним и тем же правилам на клиенте (чтобы показать ошибку сразу)
 * и на Worker (потому что клиенту верить нельзя).
 * -------------------------------------------------------------------------- */
